...
# ==================== SECTION 2: BUILD HISTORICAL FUNDAMENTALS ====================
print("\n[2/6] Fetching current fundamentals & building historical proxy...")
...
# NOTE: Dans un setup réel, on utiliserait une API comme FactSet, Bloomberg, ou Compustat
# pour obtenir les ratios historiques à chaque date. Ici, on simule en appliquant des variations.
...
print("\n[3/6] Building cross-sectional dataset (date, ticker) → features, target...")
...
def build_cross_sectional_dataset(prices, returns, fundamentals_dict, horizon=3):
    ...
    - target: rendement futur sur horizon
    """
    data_rows = []
    ...
    for date in returns.index[:-horizon]:  # On exclut les dernières dates sans futur
        for ticker in returns.columns:
            ...
            if len(future_rets) < horizon:
                continue
            target = future_rets.sum()  # Log returns → sum = cumulative
            
            # Features fondamentales (dans un setup réel: historiques à cette date)
            row = {
                'date': date,
                'ticker': ticker,
                'pe': fund.get('pe'),
...
dataset = dataset.dropna()  # Supprimer les lignes avec des NaN
...
# ==================== SECTION 4: TRAIN-TEST SPLIT TEMPOREL ====================
...
print(f"✓ Train set: {len(X_train)} observations ({X_train.index.get_level_values('date').min()} → {X_train.index.get_level_values('date').max()})")
print(f"✓ Test set:  {len(X_test)} observations ({X_test.index.get_level_values('date').min()} → {X_test.index.get_level_values('date').max()})")

# Standardisation basée sur le train set
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled = scaler.transform(X_test)

# ==================== SECTION 5: TRAIN ML MODEL (SINGLE MODEL) ====================
print("\n[5/6] Training single cross-sectional model...")
...
# ==================== SECTION 6: DYNAMIC PORTFOLIO CONSTRUCTION ====================
print("\n[6/6] Building dynamic ML portfolio with monthly rebalancing...")
...
# Construire les poids du portefeuille ML
weights_ml = {}
current_holdings = []

for rb_date in rebalance_dates:
    # Prédictions disponibles à cette date
    date_preds = test_df.loc[rb_date].sort_values('prediction', ascending=False)
    
    # Top N stocks
    top_stocks = date_preds.head(TOP_N_STOCKS).index.tolist()
    
    # Equal weight
    w = pd.Series(1.0 / TOP_N_STOCKS, index=top_stocks)
    
    # Appliquer ces poids jusqu'au prochain rebalancing
    next_idx = rebalance_dates.index(rb_date) + 1
    if next_idx < len(rebalance_dates):
        end_date = rebalance_dates[next_idx]
...
print("\n[7/7] Running backtest with transaction costs...")

def backtest_dynamic(weights_dict, returns, costs_bps=20):
    """Backtest avec rebalancing dynamique"""
    ...
    for d in dates_bt:
        ...
        # Transaction costs
        if prev_w is None:
            tc = (costs_bps / 10000.0) * w.abs().sum()  # Initial entry
        else:
    ...
    logret = pd.Series(equity, index=dates_bt)
    return np.exp(logret.cumsum())

# Benchmarks statiques (pour comparaison sur même période)
test_start = test_dates[0]
test_rets = rets.loc[test_start:]

# Value/Growth basé sur fundamentals actuels
...
# Poids constants pour benchmarks
...
_base1(eq_ml).rename("ML DYNAMIC (Top10)").plot(ax=ax, linewidth=2.5, linestyle='-', color='darkgreen')
...
# 2. Drawdowns
ax = axes[0, 1]
for eq, name, style in [(eq_ml, "ML DYNAMIC", '-'), (eq_val, "VALUE", '--'), 
                         (eq_grw, "GROWTH", '--'), (eq_mkt, "MARKET", ':')]:
...
# Holdings overlap between rebalances
overlaps = []
for i in range(len(current_holdings)-1):
    curr = set(current_holdings[i]['holdings'])
    next_hold = set(current_holdings[i+1]['holdings'])
    overlap = len(curr.intersection(next_hold)) / TOP_N_STOCKS
